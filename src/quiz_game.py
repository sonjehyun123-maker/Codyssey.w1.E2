...
class QuizGame:
    ...
    # 기본 퀴즈 5개
    DEFAULT_QUIZZES = [
        Quiz("print(2 + 3 * 2) ?", ["10", "8", "7", "12"], 2),
        Quiz("다음중 리스트는??", ["'1, 2, 3'", "{1, 2, 3}", "(1, 2, 3)", "[1, 2, 3]"], 4),
        Quiz("1GB의 크기는?", ["100", "1000", "1024", "1234"], 3),
        Quiz("print(2 ** 3) ?", ["5", "6", "7", "8"], 4),
        Quiz("print(\"2\" + \"3\")", ["5", "8", "18", "23"], 4),
    ]

    def __init__(self):
        self.quizzes = []
        self.best_score = 0
        self.load()  # 시작할 때 데이터를 불러옵니다.
